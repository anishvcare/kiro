/**
 * Public Shop Profile Page (no authentication required)
 * Guests can view shop details. Send Request and Chat buttons prompt login.
 */
...
const PublicShopProfile = () => {
  ...
  const handleSendRequest = () => {
    if (isAuthenticated) {
      navigate(`/customer/create-request/${id}`);
    } else {
      navigate('/login', { state: { from: { pathname: `/customer/create-request/${id}` } } });
    }
  };

  const handleChat = () => {
    if (isAuthenticated) {
      navigate(`/customer/chat?shopId=${id}`);
    } else {
      navigate('/login', { state: { from: { pathname: `/customer/chat?shopId=${id}` } } });
    }
  };
      ...
      <div className="min-h-screen bg-gray-50">
        <header className="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-30">
          <div className="max-w-4xl mx-auto flex items-center justify-between h-14 px-4">
            <Link to="/" className="text-lg font-bold text-indigo-600">LocalShop</Link>
            <Link to="/login" className="px-4 py-2 text-sm font-medium text-indigo-600 border border-indigo-600 rounded-lg hover:bg-indigo-50 transition-colors">Login</Link>
          </div>
        </header>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="p-4 max-w-4xl mx-auto space-y-6">
        ...
        <div className="relative">
          {shop.banner_url ? (
            <img src={shop.banner_url} alt={`${shop.name} banner`} className="w-full h-48 object-cover rounded-lg" />
          ) : (
            <div className="w-full h-48 bg-gradient-to-r from-indigo-500 to-purple-500 rounded-lg" />
          )}
          <div className="absolute -bottom-8 left-6">
            {shop.logo_url ? (
              <img src={shop.logo_url} alt={shop.name} className="w-20 h-20 rounded-full border-4 border-white object-cover shadow" />
            ) : (
              <div className="w-20 h-20 rounded-full border-4 border-white bg-indigo-100 flex items-center justify-center shadow">
                <span className="text-indigo-600 font-bold text-2xl">{shop.name?.charAt(0)?.toUpperCase()}</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="pt-10 px-4">
          ...
          {shop.category && <p className="text-sm text-gray-500 mt-1">{shop.category.name}</p>}
          ...
          {shop.description && <p className="text-gray-600 mt-3">{shop.description}</p>}
        </div>
          ...
          {shop.keywords && shop.keywords.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-2">
              {shop.keywords.map((kw, idx) => (
                <span key={idx} className="px-2 py-0.5 bg-gray-100 text-gray-600 rounded-full text-xs">{kw.keyword}</span>
              ))}
            </div>
          )}
        </div>
        ...
        {!isAuthenticated && (
          <p className="text-center text-sm text-gray-500">
            <Link to="/login" className="text-indigo-600 font-medium hover:underline">Login</Link> or{' '}
            <Link to="/register" className="text-indigo-600 font-medium hover:underline">Register</Link>{' '}
            to send requests and chat with this shop.
          </p>
        )}
      </main>
    </div>
...
export default PublicShopProfile;
